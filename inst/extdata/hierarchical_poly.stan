		...
		
	{
		...
		for(i in 1:C) {			
			for (j in 1:S[i]) {
				...
				alpha_re[k] = alpha0 - tau[1] * alphaC[i] - tau[2] * alphaS[k];
				beta_re[k]  = beta0  + tau[3] * betaC[i]  + tau[4] * betaS[k];
...
generated quantities {
    ...
    vector[N] y_sim = rep_vector(0.0, N);
    
 	{
        ...
        // standard error for sum of random effects
		real tau_alpha = pow(square(tau[1]) + square(tau[2]), 0.5);
		real tau_beta  = pow(square(tau[3]) + square(tau[4]), 0.5);
        
        // standard error for predicted decline
		tau_sum = T * square(tau_alpha);
		for (i in 1:(Y - start)) {
			tau_sum += square(i * tau_beta);
		}
		tau_sum = pow(tau_sum, 0.5);
        ...
        expected_decline = 1.0 - exp(alpha0 * T - beta0 * Tplus + square(tau_sum) / 2.0);
	}
